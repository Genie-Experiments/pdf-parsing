...
def _is_python_code(text: str) -> bool:
    ...
    return (re.search(python_keywords, text, re.I) or 
            any(re.search(pattern, text, re.MULTILINE) for pattern in python_patterns))
...
def _normalize_existing_indentation(lines: List[str]) -> List[str]:
    ...
    valid_indents = [spaces for spaces, content in line_info if spaces is not None and content]
...
def _infer_python_indentation(lines: List[str]) -> List[str]:
    ...
    for i, line in enumerate(lines):
        ...
        # Handle return statements - usually at function body level
...
def _remove_excessive_blank_lines(lines: List[str], max_consecutive: int = 2) -> List[str]:
...
def _is_cli_config(text: str) -> bool:
    ...
    return bool(re.search(r"\b(configure terminal|interface|router|ip address|vlan|spbm)\b", text, re.I))
...
def _is_key_value_content(text: str) -> bool:
    ...
    python_keywords = r"\b(def|class|import|from|if|elif|else|for|while|try|except|finally|with)\b"
...
def _format_key_value_content(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        if (":" in line_stripped and 
            not re.search(r"^\s*(if|elif|else|for|while|try|except|finally|with|def|class).*:", line_stripped)):
