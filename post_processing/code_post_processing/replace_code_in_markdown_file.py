    ...
    try:
        ...
        for i, match in enumerate(code_blocks):
            ...
            if similarity > 0.7:  # 70% similarity threshold for code blocks only
                log_success(f"Found fuzzy match in code block {i+1} with {similarity:.2f} confidence", logger)
                content = content.replace(block_content, new_code.strip(), 1)  # Replace only first occurrence
                replacements_made += 1
                break
        ...
        if replacements_made > 0:
            ...
            with open(markdown_file_path, 'w', encoding='utf-8') as f:
                f.write(content)
            log_success(f"Successfully made {replacements_made} replacement(s) in markdown file", logger)
