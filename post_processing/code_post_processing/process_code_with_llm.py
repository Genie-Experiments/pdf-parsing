        ...
        try:
            ...
            for page in data.get("pages", []):
                for page_element in page.get("elements", []):
                    if (page_element.get("bbox") == element.get("bbox") and
...
code_processor = LLMCodeProcessor()


def process_code_with_llm(element: Dict[str, Any], json_file_path: str) -> Optional[str]:
    """
    ...
    Returns:
        Extracted and formatted code string, or None if processing fails
    """
    return code_processor.process_code_with_llm(element, json_file_path)