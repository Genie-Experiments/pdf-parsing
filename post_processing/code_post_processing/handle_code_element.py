...
def _resolve_file_paths(json_file_path: str, logger) -> Tuple[Optional[str], Optional[Path]]:
    """
    ...
    if not pdf_file_path:
        log_warning("Could not find corresponding PDF file - cannot perform PDF-based code replacement", logger)
        return None, None
...
def _extract_and_compare_code(pdf_file_path: str, page_number: int, original_text: str, logger) -> str:
    """
    ...
    if extracted_code != original_text:
        ...
        logger.info("Original length: %d chars → PDF length: %d chars", 
                   len(original_text), len(extracted_code))
        logger.debug("PDF Code preview: %s", 
                    extracted_code[:100] + "..." if len(extracted_code) > 100 else extracted_code)
        return extracted_code
    else:
        logger.info("PDF extraction returned same code (no improvement found)")
        return original_text

def _replace_code_in_markdown(markdown_file_path: Path, cleaned_code: str, final_code: str, logger) -> bool:
    """
...
def _process_code_with_pdf(text: str, pdf_file_path: str, markdown_file_path: Path, 
                          page_number: int, logger) -> bool:
    """
    ...
    try:
        ...
        success = _replace_code_in_markdown(markdown_file_path, cleaned_code, final_code, logger)
        
        if success:
            logger.info("Individual code block processing completed for page %d", page_number)
            return True
        else:
            return False
...
def handle_code_element(text: str, json_file_path: str, page_number: int, 
                       bbox: list, reading_order: int, label: str) -> bool:
    """
    ...
    return _process_code_with_pdf(text, pdf_file_path, markdown_file_path, page_number, logger)