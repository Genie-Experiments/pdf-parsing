...
def extract_code_from_pdf_page(pdf_path: str, page_number: int, target_code: str) -> str:
    ...
    try:
        logger.debug("Opening PDF and extracting text from page %d (PDF index %d)", 
        ...
        if page_number >= len(pdf_doc):
            log_warning(f"Page {page_number + 1} not found in PDF (total pages: {len(pdf_doc)})", logger)
            pdf_doc.close()
            return target_code
        ...
        if best_match and best_score > 0.6:  # 60% similarity threshold
            ...
            logger.debug("Matched code preview: %s", best_match[:100] + "..." if len(best_match) > 100 else best_match)
