...
def compare_markdown_files(file1_path, file2_path, save_json=True):
    ...
    print("-" * 80)
    print(
        f"File1 text length:    {len(file1_text)} characters, {len(file1_words)} words"
    )
    print(
        f"File2 text length:    {len(file2_text)} characters, {len(file2_words)} words"
    )
    print(
        f"Length difference:    {abs(len(file1_text) - len(file2_text))} characters, {abs(len(file1_words) - len(file2_words))} words"
    )
    ...
    if errors:
        ...
        for i, error in enumerate(errors[:20], 1):
            ...
            print(
                f"   Position: File1 word {error['position_gold']}, File2 word {error['position_ocr']}"
            )
            if error["gold_text"]:
            ...
            print(
                f"   Context:  ...{error['context_before_gold']} [{error['gold_text'] or 'DELETED'}] {error['context_after_gold']}..."
            )
            print(
                f"             ...{error['context_before_ocr']} [{error['ocr_text'] or 'MISSING'}] {error['context_after_ocr']}..."
            )
    ...
    print("-" * 80)
    print(
        "This is a direct comparison of the two markdown files with no preprocessing or normalization applied."
    )
    print("=" * 80)
