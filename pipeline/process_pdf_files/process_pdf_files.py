...
# Absolute path to the pipeline root (this file lives at pipeline/process_pdf_files/)
_PIPELINE_ROOT = Path(__file__).parent.parent


def process_pdf_files(
    data_dir, results_dir, dolphin_script, model_path, max_batch_size=16
):
    """
    ...
    for pdf_file in pdf_files:
        try:
            ...
            # Dolphin page images go under the per-PDF output dir
            dolphin_images_dir = output_dir / "Dolphin" / "imgs_pages"
            ...
            # Execute the command from the pipeline root so Dolphin's relative
            # imports and data paths resolve correctly regardless of cwd at launch.
            result = subprocess.run(
                cmd, capture_output=True, text=True, cwd=str(_PIPELINE_ROOT)
            )
            ...
            if result.returncode == 0:
                ...
                log_error(
                    f"Dolphin failed for {pdf_name} (exit {result.returncode}): {stderr}"
                )
                raise RuntimeError(
                    f"Dolphin inference failed for '{pdf_name}' (exit {result.returncode}). "
                    f"stderr: {stderr}"
                )

        except RuntimeError:
            raise  # propagate Dolphin failures — don't silently continue
