...
def detect_content_type(text: str) -> str:
    ...
    # Count matches
    code_score = sum(
        1 for pattern in code_indicators if re.search(pattern, text_sample)
    )
    command_score = sum(
        1 for pattern in command_indicators if re.search(pattern, text_sample)
    )
    config_score = sum(
        1 for pattern in config_indicators if re.search(pattern, text_sample)
    )
    ...
    if len(text) > 50:  # Only log for substantial content
        ...
        logger.debug(
            "Content type detection - scores: code=%d, command=%d, config=%d → %s",
            code_score,
            command_score,
            config_score,
            max_type if scores[max_type] >= 2 else "mixed/natural",
        )
...
def fuzzy_match_similarity(text1: str, text2: str, adaptive: bool = True) -> float:
    ...
    if content_type == "natural_language" and similarity < 0.7:
        ...
        if words1 and words2:
            ...
            similarity = max(
                similarity, word_similarity * 0.8
            )  # Slight penalty for word-only match
...
def normalize_text_for_comparison(text: str, aggressive_mode: bool = False) -> str:
    ...
    # Multi-character OCR corrections (safe — only replace whole-token lookalikes).
    # Single-character replacements like "l"→"1" or "S"→"5" were removed because
    # they corrupt every word containing those letters (e.g. "list"→"1ist").
    if aggressive_mode:
        ocr_corrections = {
            "rn": "m",
            "ii": "ll",
...
def get_context_elements(
    json_data: Dict[str, Any], code_element: Dict[str, Any], context_window: int = 3
) -> List[Dict[str, Any]]:
    """
    ...
    for page in json_data.get("pages", []):
        for element in page.get("elements", []):
            if (
                element.get("reading_order") == target_reading_order
                and element.get("label") == "code"
            ):
                target_page = page
                break
        if target_page:
    ...
    page_elements = sorted(
        target_page.get("elements", []), key=lambda x: x.get("reading_order", 0)
    )
...
def search_code_in_pdf(
    ...
    try:
        ...
        if min_confidence is None:
            ...
            logger.debug(
                "Auto-detected content type '%s', using confidence threshold: %.2f",
                content_type,
                min_confidence,
            )
        else:
        ...
        for page_num in range(len(pdf_doc)):
            ...
            page_matches = _match_with_strategies(
                ...
                min_confidence,
                page,
                detect_content_type(code_content),
            )
            matches.extend(page_matches)

            if page_matches:
                logger.debug(
                    "Found %d matches on page %d", len(page_matches), page_num + 1
                )
    ...
    if matches:
        logger.info(
            "Best match has confidence score: %.3f", matches[0].confidence_score
        )
    ...
    for match in matches:
        ...
        for existing in unique_matches:
            if (
                existing.page_number == match.page_number
                and abs(existing.confidence_score - match.confidence_score) < 0.1
            ):
                is_duplicate = True
                break
        if not is_duplicate:
...
def _match_with_strategies(
    ...
    if context_text and len(context_text.strip()) > 10:
        ...
        ):  # Slightly lower threshold for context matches
            matches.append(
                CodeMatch(
                    page_number=page_num,
    ...
    if len(code_lines) > 1:
        ...
        if best_match_score >= min_confidence:
            matched_text = "\n".join(
                page_lines[best_match_start : best_match_start + len(code_lines)]
            )
            matches.append(
                CodeMatch(
                    page_number=page_num,
                    confidence_score=best_match_score
                    * 0.95,  # High priority for line matching
