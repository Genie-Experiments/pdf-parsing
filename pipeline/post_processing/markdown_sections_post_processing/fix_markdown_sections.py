...
def build_section_to_level_map(
    hierarchy: Dict, current_level: int = 1
) -> Dict[str, int]:
    """
...
def fix_markdown_sections(
    markdown_file: str, json_file: str, output_file: str = None, in_place: bool = False
):
    """
        ...
        if match:
            ...
            if heading_text in section_to_level:
                ...
                if len(new_hashes) != len(current_hashes):
                    logger.info(
                        f"Fixed: '{heading_text}' (Level {len(current_hashes)} → {correct_level})"
                    )
            else:
                # Keep original if not found in hierarchy
                fixed_lines.append(line)
                logger.debug(
                    "No match found for heading: '%s' (keeping original)", heading_text
                )
        else:
    ...
    match_rate = (fixed_headings / total_headings * 100) if total_headings > 0 else 0.0
...
def batch_fix_markdown_sections(toc_json_dir: str, results_dir: str):
    ...
    logger.info(
        "Starting batch processing of markdown section hierarchy (Simple Mode)..."
    )
    ...
    for json_file in json_files:
        try:
            ...
            fix_markdown_sections(str(markdown_file), str(json_file), in_place=True)
