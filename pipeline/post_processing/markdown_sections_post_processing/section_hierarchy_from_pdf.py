...
def batch_process_pdfs(
    data_directory=None,
    output_base_dir="./Results/_pipeline/section_hierarchy",
    min_heading_size=None,
    max_levels=6,
    bold_only=False,
    exclude_headers_footers=True,
    ...
    for i, (pdf_path, relative_path) in enumerate(pdf_files, 1):
        ...
        try:
            ...
            # Check if output already exists — overwrite silently (no interactive prompt)
            if os.path.exists(output_json_path):
                logger.debug(f"JSON already exists, overwriting: {output_json_path}")
