...
class FigureProcessor:
    ...
    def _gather_section_context(
        self, figure_element: Dict[str, Any], page_elements: List[Dict[str, Any]]
    ) -> str:
        """
        ...
        try:
            ...
            sorted_elements = sorted(
                page_elements, key=lambda x: x.get("reading_order", 0)
            )
            ...
            if len(context_text) > settings.max_context_length:
                ...
                logger.info(
                    f"Context truncated to {settings.max_context_length} characters"
                )
    ...
    def _find_section_start(
        self, sorted_elements: List[Dict[str, Any]], figure_reading_order: int
    ) -> int:
    ...
    def _find_section_end(
        ...
        for i, elem in enumerate(sorted_elements):
            if (
                elem.get("reading_order", 0) > figure_reading_order
                and elem.get("label") in section_labels
            ):
                return i
    ...
    def _extract_section_text(
        self, sorted_elements: List[Dict[str, Any]], start_idx: int, end_idx: int
    ) -> List[str]:
...
class VisionDescriptionGenerator:
    ...
    def generate_vision_description(
        self, figure_path: str, context_text: str
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if response:
                ...
                log_success(
                    "Generated description using OpenAI Vision (actual image analysis)"
                )
                return response
    ...
    def _call_vision_api(self, prompt: str, base64_image: str) -> Optional[str]:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                temperature=0.3,
                max_tokens=settings.max_description_length,
            )
    ...
    def generate_context_description(
        ...
        try:
            ...
            if response:
                ...
                log_success(
                    "Generated description using OpenAI context-based approach (text only)"
                )
                return response
...
class FigureDescriptionManager:
    ...
    def generate_description(
        ...
        logger.info(
            "Using context-based description (vision analysis failed or unavailable)"
        )
...
class MarkdownIntegrator:
    ...
    def search_and_replace_figure(
        self, figure_text: str, description: str, json_file_path: str
    ) -> bool:
        """
    ...
    def _replace_figure_text(
        self, content: str, figure_text: str, description: str
    ) -> str:
...
def process_figure_element(
    element: Dict[str, Any], page_elements: List[Dict[str, Any]], json_file_path: str
) -> bool:
    """
...
def get_figure_info_and_context(
    ...
    return processor.extract_figure_info_and_context(
        element, page_elements, json_file_path
    )


def search_and_replace_figure_in_markdown(
    figure_text: str, description: str, json_file_path: str
) -> bool:
    """Legacy function for backward compatibility."""
    integrator = MarkdownIntegrator()
    return integrator.search_and_replace_figure(
        figure_text, description, json_file_path
    )
