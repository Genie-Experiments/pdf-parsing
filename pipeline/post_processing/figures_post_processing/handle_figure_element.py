...
def handle_figure_element(text, element, page, json_file_path):
    ...
    figure_info = get_figure_info_and_context(
        element, page.get("elements", []), json_file_path
    )

    if figure_info["image_exists"]:
        log_success(
            f"Figure image found: {figure_info['relative_figure_path']}", logger
        )
        ...
        description = generate_figure_description(
            figure_info["figure_path"], figure_info["context_text"]
        )
        ...
        if description:
            ...
            success = search_and_replace_figure_in_markdown(
                figure_info["figure_text"], description, json_file_path
            )

            if success:
                log_success(
                    "Figure text successfully replaced with description!", logger
                )
            else:
        ...
        log_error(
            f"Figure image not found: {figure_info['relative_figure_path']}", logger
        )
