...
def load_json_data(json_file):
    ...
    try:
        ...
        raise ValueError(f"Error parsing JSON file '{json_file}': {e}") from e
...
def remove_text_from_content(content, text_to_remove):
    ...
    while i < len(lines):
        ...
        is_heading_with_text = re.match(
            r"^\s*#+\s+", line
        ) and normalized_text_to_remove in normalize_text_for_comparison(line)
        ...
        if is_heading_with_text:
            ...
            if normalized_line == normalized_text_to_remove:
                ...
                if normalized_line and normalized_text_to_remove.startswith(
                    normalized_line
                ):
                    ...
                    while j < len(lines) and len(combined_text) < len(
                        normalized_text_to_remove
                    ):
                        next_line = lines[j]
...
def _apply_header_footer_removals(markdown_content: str, headers_footers: dict):
    """
    Apply header, footer and watermark removals to markdown content.

    Returns:
        Tuple of (updated_content, total_removals)
    """
    total_removals = 0
    ...
    for page_num in sorted(headers_footers.keys()):
        ...
        if page_num == 1:
            ...
            # For other pages, remove both headers and footers
        ...
        if changes_on_page > 0:
            updated_content = update_page_content(
                updated_content, page_num, page_content
            )
            total_removals += changes_on_page

    return updated_content, total_removals
...
def remove_headers_footers(markdown_file, json_file, output_file=None):
    ...
    updated_content, total_removals = _apply_header_footer_removals(
        markdown_content, headers_footers
    )
