...
def get_json_files_list(results_directory):
    ...
    # Find all Dolphin recognition JSON files, excluding:
    #   _pipeline/  — section hierarchy and other internal pipeline artifacts
    #   _corrections.json — OCR correction logs written by fix_ocr_errors
    json_files = [
        p
        for p in results_path.rglob("*.json")
        if "_pipeline" not in p.parts and not p.name.endswith("_corrections.json")
    ]
