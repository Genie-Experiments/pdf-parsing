...
def create_markdown_backup(source_dir, backup_suffix="_backup"):
    ...
    logger.info(
        "Creating backup of markdown files in '%s' with suffix '%s'",
        source_dir,
        backup_suffix,
    )
    ...
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            if file.lower().endswith(".md") and not file.endswith(
                f"{backup_suffix}.md"
            ):
    ...
    try:
        ...
        log_success(
            f"Successfully backed up {copied_count} markdown files with '{backup_suffix}' suffix",
            logger,
        )
        return copied_count
